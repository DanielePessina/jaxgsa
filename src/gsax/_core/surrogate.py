...
exported from ``gsax`` and user code should type against ``PCEResult`` /
``HDMRResult`` directly.
"""
...
from gsax._core.validation import _validate_x
...
class SurrogateResult(ABC):
    ...
    ``gsax.shapley._analyze``); :meth:`predict` is implemented once here.
    """
