...
Reproduces the full-N fit in :mod:`gsax.hdmr._engine` without ever holding
...
from gsax.hdmr._engine import _build_B1, _build_B2, _build_B3
...
def _get_gram_backfit(m1: int, n1: int, maxiter: int, lambdax: float):
    ...
    Rewrites :func:`gsax.hdmr._engine._fit_first_order` in coefficient space.
...
def _get_gram_higher_solver(m_basis: int, lambdax: float):
    ...
    Mirrors :func:`gsax.hdmr._engine._fit_higher_order`: each term is solved
