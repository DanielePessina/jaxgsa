...
def sample(
    ...
    *,
    ...
    Warns:
        JaxgsaWarning: If the resolved ``base_n`` is below 16. The design is
            then degenerate and the indices (and any bootstrap) are
            unreliable; this is a warning, not an error, because a tiny
            design may be a deliberate probe.
    """
    ...
    # A base count this small has one point per group at best: the variance
    # estimates sit on a handful of groups, and the bootstrap resamples a
    # near-empty pool. The design still runs (a user may legitimately probe a
    # tiny design), but it is degenerate, so say so right after resolution
    # rather than after the estimates come back meaningless.
    if base_n < 16:
        warnings.warn(
            f"jaxgsa.sobol.sample: base_n={base_n} is below the floor of 16 "
            "base points. The design is degenerate: the Sobol indices will be "
            "unreliable, and the bootstrap will resample a near-empty pool. "
            "Raise n_samples (or base_n) for a usable design.",
            stacklevel=2,
            category=JaxgsaWarning,
        )
