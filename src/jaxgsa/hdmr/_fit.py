...
def _get_finalize(n: int, n1: int, n2: int, m1: int, m2: int, m3: int, N: int):
    ...
    Returns:
        A jitted function ``(V_Y, acc, f_crits) -> (Sa, Sb, S, select, rmse)``
        over every lane at once. It is elementwise in the lane axis, so it
        does not need chunking.
    """
    ...
    def finalize(
        ...
        acc_var_f, acc_cov_fy, acc_cov_fy0m, acc_ssr, acc_cross, acc_fsq, acc_res2 = acc
        # A constant-output slice has V_Y == 0, so every index is 0/0. Emit
        # NaN (matching Sobol and the zero-variance warning) rather than
