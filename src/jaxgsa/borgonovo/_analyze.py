...
def analyze(
    ...
    *,
    ...
    Returns:
        ...
        ``delta = S1 = 0``, where SALib raises an error, and so does a
        parameter that is constant in the sample: a constant input cannot
        shift the output distribution, so its indices are provably 0. A
        conditioning class the output grid cannot resolve gets a floored KDE
        bandwidth
    ...
    Warns:
        JaxgsaWarning: On any of six conditions. An output slice has zero
            variance, so every conditional density equals the unconditional
            one and delta is an exact 0 rather than an answer. A parameter is
            constant in the sample, so its delta and S1 are exactly 0 — a
            constant input cannot shift the output distribution. A passed
    ...
    # A constant input cannot shift the output distribution, so its delta
    # and S1 are provably 0 and are forced to exactly 0 below; the plug-in
    # estimate only approaches that by sampling noise. Detected on the host,
    # where X is concrete; the kernel then masks on the passed-in flag.
    const_inputs = np.max(np.asarray(X), axis=0) == np.min(np.asarray(X), axis=0)
    const_dims = np.flatnonzero(const_inputs)
    if const_dims.size > 0:
        names = ", ".join(repr(problem.names[d]) for d in const_dims)
        plural = "s" if const_dims.size > 1 else ""
        are = "are" if const_dims.size > 1 else "is"
        warnings.warn(
            f"jaxgsa.borgonovo: parameter{plural} {names} {are} constant in the "
            "sample, so delta and S1 are exactly 0 — a constant input cannot "
            "shift the output distribution",
            stacklevel=2,
    ...
    # Force the provably-zero parameters to exactly 0 on every replicate, so
    # the point estimate, the bootstrap and the intervals all agree.
    const_cols = jnp.asarray(const_inputs, dtype=bool)
    d_all = jnp.where(const_cols, 0.0, d_all)
    s1_all = jnp.where(const_cols, 0.0, s1_all)
...
def indices(
    ...
    *,
    ...
    rather than the model. A parameter that is constant in the sample still
    yields exactly 0, computed traceably, as in :func:`analyze`; only the
    warning is missing here.
    ...
    # Same provably-zero mask as analyze: a constant input cannot shift the
    # output distribution. Computed traceably, so indices stays jittable.
    const_cols = jnp.max(X, axis=0) == jnp.min(X, axis=0)
    return jnp.where(const_cols, 0.0, delta), jnp.where(const_cols, 0.0, S1)
