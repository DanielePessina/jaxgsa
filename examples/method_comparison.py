...
def _run_all(gsax, ishigami, ishigami_fn, jax, jnp, problem, time):
    ...
    _X_hdmr = jax.random.uniform(
        _key, (2000, 3), minval=_bounds[:, 0], maxval=_bounds[:, 1]
    )
...
def _s1_chart(
    ...
    _ax.scatter(
        _x, _s1_ana, marker="D", color="black", s=60, zorder=5, label="Analytical"
    )
...
def _st_chart(
    ...
    _ax.scatter(
        _x, _st_ana, marker="D", color="black", s=60, zorder=5, label="Analytical"
    )
...
def _cost_scatter(
    ...
    for _m, _n, _e, _c in zip(
        _methods, _n_evals, _mae_st, _colors, strict=True,
    ):
        _ax.scatter(
            _n, _e, s=120, color=_c, zorder=5, edgecolors="white", linewidth=1.2,
        )
        _ax.annotate(
            _m,
            (_n, _e),
