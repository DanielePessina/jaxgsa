...
def _imports():
    ...
    return (
        gsax, jax, jnp, json, mo, np, os, plt, sample_mc, time, warnings,
...
@app.cell
def _config(gsax, jnp, np, os, time, jax):
    D_PARAMS = 5
    N_REPEATS = 3
    BASE_N = 512
...
def _benchmark(
    ...
    # Load cache if available and button not pressed
    ...
    for _label, _T, _K in SCENARIOS:
        ...
        _g = best_of(lambda sr=_sr, Y=_Yj: gsax.analyze(sr, Y)) * 1e3

        # SALib manual point-estimate path
        def _run_salib_sobol(Y=_Ys, T=_T, K=_K):
            _slices = (
                [Y] if T == 1 and K == 1
                else [Y[:, k] for k in range(K)] if T == 1
                else [Y[:, t, k] for t in range(T) for k in range(K)]
            )
            for _sl in _slices:
        ...
        _run_salib_sobol()
        _s = min(
            (time.perf_counter(), _run_salib_sobol(), time.perf_counter())[2]
            - (time.perf_counter(), _run_salib_sobol(), time.perf_counter())[0]
            for _ in range(1)
        )
        # proper timing
        _sb = float("inf")
        ...
        _all.append({"method": "Sobol", "scenario": _label, "gsax_ms": _g, "salib_ms": _s})
    ...
    for _label, _T, _K in SCENARIOS:
        ...
        # Reshape for gsax: (N*D, T) -> (N*D, T, 1) for time-series
        _Yin = _Ye[..., None] if _Ye.ndim == 2 and _T > 1 else _Ye

        gsax.analyze_efast(BENCH_PROBLEM, _Yin, M=4).S1.block_until_ready()
        _g = best_of(
            lambda Y=_Yin: gsax.analyze_efast(BENCH_PROBLEM, Y, M=4),
        ) * 1e3

        _Xsf = salib_fast_sampler.sample(SALIB_PROBLEM, 2049, M=4, seed=42)
        _Ysf = np.asarray(coupled_oscillators(jnp.asarray(_Xsf), _T, _K))
        ...
        def _run_salib_fast(Y=_Ysf, T=_T, K=_K):
            ...
            for _sl in _slices:
                salib_fast_mod.analyze(
                    ...
                    num_resamples=0, print_to_console=False,
                )
        ...
        _all.append({"method": "eFAST", "scenario": _label, "gsax_ms": _g, "salib_ms": _s})

    # === DGSM ===
    for _label, _T, _K in SCENARIOS:
        _Xmc = jnp.asarray(sample_mc(BENCH_PROBLEM, 2048, seed=42))
        _fn = make_unbatched(_T, _K)

        gsax.analyze_dgsm(BENCH_PROBLEM, _fn, _Xmc).nu.block_until_ready()
        _gb = float("inf")
        for _ in range(N_REPEATS):
            _t0 = time.perf_counter()
            gsax.analyze_dgsm(BENCH_PROBLEM, _fn, _Xmc).nu.block_until_ready()
        ...
        _Xfd = salib_finite_diff.sample(SALIB_PROBLEM, 2048, delta=0.01)
        _Yfd = np.asarray(coupled_oscillators(jnp.asarray(_Xfd), _T, _K))
        ...
        _all.append({"method": "DGSM*", "scenario": _label, "gsax_ms": _g, "salib_ms": _s})

    # === HDMR ===
    _rng = np.random.default_rng(42)
    _bounds = np.array(BENCH_PROBLEM.bounds)
    _Xnp = _rng.uniform(_bounds[:, 0], _bounds[:, 1], size=(BASE_N, D_PARAMS))
    ...
    for _label, _T, _K in SCENARIOS:
        ...
        gsax.analyze_hdmr(BENCH_PROBLEM, _Xjax, _Yj, maxorder=2, m=2).Sa.block_until_ready()
        _g = best_of(
            lambda X=_Xjax, Y=_Yj: gsax.analyze_hdmr(BENCH_PROBLEM, X, Y, maxorder=2, m=2),
        ) * 1e3
        ...
        with warnings.catch_warnings():
            ...
            _slices = (
                ...
                else [_Ynp[:, t, k] for t in range(_T) for k in range(_K)]
            )
            for _sl in _slices:
                salib_hdmr_mod.analyze(
                    SALIB_PROBLEM, _Xnp, _sl,
                    maxorder=2, maxiter=100, print_to_console=False,
                )
        _s = (time.perf_counter() - _t0) * 1e3

        _all.append({"method": "HDMR", "scenario": _label, "gsax_ms": _g, "salib_ms": _s})
...
def _table(all_results, mo):
    ...
    _header = (
        "| Method | Scenario | gsax (ms) | SALib (ms) | Speedup |\n"
        "| --- | --- | ---: | ---: | ---: |"
    )
    _lines = [_header]
    ...
    mo.md(
        "## Results\n\n"
        "All timings are best-of-3 wall-clock ms (analysis only, "
        "model evaluation excluded). DGSM* gsax times include "
        "autodiff (inherent to the method).\n\n"
        + "\n".join(_lines)
    )
    return
...
def _speedup_chart(all_results, np, plt):
    ...
    _width = 0.18
    _offsets = [(_i - 1.5) * _width for _i in range(4)]
    ...
    for _si, _sc in enumerate(_scenarios):
        ...
        _ax.bar(
            _x + _offsets[_si], _vals, _width,
            label=_sc, color=_colors[_si], alpha=0.85,
        )

    _ax.axhline(1, color="black", linewidth=0.8, linestyle="--", alpha=0.5)
    _ax.set_xticks(_x)
    _ax.set_xticklabels(_methods)
    _ax.set_ylabel("Speedup (SALib / gsax)")
    _ax.set_title("gsax speedup over SALib by method and scenario")
...
def _scaling_chart(all_results, plt):
    ...
    for _m in _methods:
        if _m not in _by_method:
            continue
        _rows = sorted(_by_method[_m], key=lambda r: _tk[r["scenario"]])
        _xs = [_tk[r["scenario"]] for r in _rows]
        _ys = [r["salib_ms"] / r["gsax_ms"] for r in _rows if r["gsax_ms"] > 0]
        _ax.plot(
            _xs, _ys, marker=_markers[_m], color=_colors[_m],
            linewidth=2, markersize=8, label=_m,
        )
...
def _summary(mo):
    ...
    - **Speedup grows with T x K** — SALib loops over output slices
      in Python; gsax vectorizes with `jax.vmap`
    - **HDMR has largest speedups** — SALib runs backfitting per
      slice; gsax vmaps the entire B-spline fit
    - **eFAST speedup is moderate** — SALib's FFT is already fast
      in NumPy; gsax gains from fused JIT + vmap
    - **DGSM is apples-to-oranges** — gsax uses autodiff (exact
      gradients); SALib uses finite differences
    - **Scalar outputs (1x1)** — SALib can be faster due to JIT
      compilation overhead in gsax
