...
pre-computed and excluded from timing. Results cache to JSON.

Run as script: ``uv run --extra dev python examples/benchmark_all.py``
Pass ``--refresh`` to re-run the benchmarks instead of loading the cache.
"""
...
class _AllResults(TypedDict):
    speed: list[_SpeedRow]
...
_ishi_bounds = ISHI_PROB.bounds
assert _ishi_bounds is not None
SALIB_ISHI = {
    "num_vars": 3,
    "names": list(ISHI_PROB.names),
    "bounds": [list(b) for b in _ishi_bounds],
}
SALIB_OAKH = {
    "num_vars": 15,
...
def ishi_batch(X, T, K):
    ...
    if T == 1:
        _ys = [
            jnp.sin(x1) + _As[k] * jnp.sin(x2) ** 2 + _B * x3**4 * jnp.sin(x1) for k in range(K)
        ]
...
# ---- Run benchmarks or load the cache ----
_parser = argparse.ArgumentParser(
    description="Benchmark jaxgsa vs SALib; load cached results unless --refresh.",
)
_parser.add_argument(
    "--refresh",
    action="store_true",
    help="Re-run the full benchmark and overwrite the cache.",
)
_args = _parser.parse_args()

if not _args.refresh and os.path.exists(CACHE_PATH):
    with open(CACHE_PATH) as _f:
        all_results: _AllResults = cast(_AllResults, json.load(_f))
else:
    _all: list[_SpeedRow] = []
    _problems = [
        (
    ...
    all_results: _AllResults = {"speed": _all, "bootstrap": _boot}
...
# ---- Bar charts: speedup per method and scenario ----
_speed = all_results["speed"]
...
# ---- Scaling chart: speedup vs output dimensionality ----
_speed = all_results["speed"]
...
if _boot is None:
    print("No bootstrap section in the cached results; skipping.")
else:
