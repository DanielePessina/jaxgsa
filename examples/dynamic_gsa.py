...
def _model(gsax, jnp):
    ...
    def oscillator(X):
        ...
        t = times[None, :]
        y = (
            amp * jnp.sin(2 * jnp.pi * freq * t) * jnp.exp(-damping * t)
            + coupling * t * jnp.exp(-damping * t)
        )
        return y
...
def _s1_plot(np, plt, problem, sobol_result, times):
    ...
    for _d, (_name, _color) in enumerate(zip(_names, _colors, strict=True)):
        ...
        _ax.fill_between(
            _t, _s1_lo[:, _d], _s1_hi[:, _d], color=_color, alpha=0.18
        )
    _ax.set_xlabel("Time")
    _ax.set_ylabel("$S_1$")
...
def _st_plot(np, plt, problem, sobol_result, times):
    ...
    for _d, (_name, _color) in enumerate(zip(_names, _colors, strict=True)):
        ...
        _ax.fill_between(
            _t, _st_lo[:, _d], _st_hi[:, _d], color=_color, alpha=0.18
        )
    _ax.set_xlabel("Time")
    _ax.set_ylabel("$S_T$")
...
def _comparison_plot(efast_result, np, plt, problem, sobol_result, times):
    ...
    for _d, (_name, _color) in enumerate(zip(_names, _colors, strict=True)):
        _axes[0].plot(
            _t, _s1_sob[:, _d], color=_color, linewidth=1.6, label=f"{_name} (Sobol)"
        )
        _axes[0].plot(
            _t, _s1_ef[:, _d], color=_color, linewidth=1.6, linestyle="--",
            label=f"{_name} (eFAST)",
        )
        _axes[1].plot(
            _t, _st_sob[:, _d], color=_color, linewidth=1.6, label=f"{_name} (Sobol)"
        )
        _axes[1].plot(
            _t, _st_ef[:, _d], color=_color, linewidth=1.6, linestyle="--",
            label=f"{_name} (eFAST)",
        )
...
def _dgsm_analysis(gsax, jnp, oscillator_unbatched, problem, sample_mc):
    ...
    dgsm_result = gsax.analyze_dgsm(
        problem, oscillator_unbatched, jnp.asarray(X_dgsm)
    )
    print(dgsm_result)
    return (dgsm_result,)
...
def _dgsm_plot(dgsm_result, np, plt, problem, sobol_result, times):
    ...
    for _d in range(_D):
        ...
        _ax.fill_between(
            _t, _lb[:, _d], _ub[:, _d],
            color=_colors[_d], alpha=0.20, label="DGSM bounds",
        )
        _ax.plot(
            _t, _st_sob[:, _d],
            color=_colors[_d], linewidth=1.8, label="Sobol $S_T$",
        )
        _ax.plot(
            _t, _ub[:, _d],
            color=_colors[_d], linewidth=0.8, linestyle=":",
        )
        _ax.plot(
            _t, _lb[:, _d],
            color=_colors[_d], linewidth=0.8, linestyle=":",
    ...
    _fig.suptitle(
        "DGSM bounds bracket Sobol $S_T$ at every time step", y=1.01
    )
    _fig.tight_layout()
    _fig
    return
