    ...
    def test_s2_near_zero(self, linear_pce_result):
        ...
        for i in range(D):
            for j in range(i + 1, D):
                assert abs(S2[i, j]) < 0.02, (
                    f"S2[{i},{j}] = {S2[i, j]:.4f}, expected ~0"
                )
...
class TestEmulatorRoundTrip:
    ...
    def test_linear_emulator_reproduces_training_data(
        self, linear_pce_result, linear_pce_data
    ):
    ...
    def test_ishigami_emulator_reasonable(
        self, ishigami_pce_result, ishigami_pce_data
    ):
...
class TestS2Properties:
    ...
    def test_diagonal_is_nan(self, linear_pce_result):
        ...
        assert np.all(np.isnan(np.diag(S2))), (
            f"S2 diagonal should be NaN, got {np.diag(S2)}"
        )
    ...
    def test_symmetric(self, linear_pce_result):
        ...
        np.testing.assert_allclose(
            S2[upper], S2[lower], atol=1e-10, rtol=1e-10
        )
...
class TestLooRmse:
    ...
    def test_loo_small_for_linear(self, linear_pce_result, linear_pce_data):
        ...
        assert loo / y_range < 0.05, (
            f"LOO RMSE / range = {loo / y_range:.4f}, expected < 0.05"
        )
    ...
    def test_loo_engine_function_matches(self):
        ...
        np.testing.assert_allclose(
            float(result.loo_rmse), float(loo_direct), rtol=1e-5
        )
...
class TestGaussianInputs:
    ...
    def test_gaussian_emulator(self, gaussian_pce_result, gaussian_pce_data):
        ...
        np.testing.assert_allclose(
            np.asarray(Y_pred), np.asarray(Y), atol=0.5, rtol=0.1
        )
    ...
    def test_hermite_1d_orthonormality(self):
        ...
        gram = (H.T @ H) / N
        np.testing.assert_allclose(
            np.asarray(gram), np.eye(4), atol=0.05
        )
    ...
    def test_legendre_1d_orthonormality(self):
        ...
        gram = (P.T @ P) / N
        np.testing.assert_allclose(
            np.asarray(gram), np.eye(5), atol=0.05
        )
...
class TestToDataset:
    ...
    def test_dataset_values_match_result(self, linear_pce_result):
        ...
        ds = linear_pce_result.to_dataset()
        np.testing.assert_allclose(
            ds["S1"].values, np.asarray(linear_pce_result.S1), rtol=1e-10
        )
        np.testing.assert_allclose(
            ds["ST"].values, np.asarray(linear_pce_result.ST), rtol=1e-10
        )
