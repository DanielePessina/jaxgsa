...
class TestEngine:
    ...
    def test_build_multi_index_survives_dimension_over_recursion_limit(self):
        ...
        for D in (996, 1000, 1200):
            mi = build_multi_index(D, 1)
            assert mi.shape == (comb(D + 1, 1), D)
            # Order 1: the constant term, then one unit vector per dimension.
            np.testing.assert_array_equal(mi[0], np.zeros(D, dtype=mi.dtype))
            np.testing.assert_array_equal(mi[1:], np.eye(D, dtype=mi.dtype)[::-1])

    def test_build_multi_index_is_graded_and_lex_ordered(self):
        """The set is ordered by total degree, then lexicographically."""
    ...
    def test_order_one_fit_matches_linear_oracle_above_recursion_limit(self):
        """A high-dimensional order-one fit recovers a known variance split."""
        D, N = 1000, 2500
        problem = Problem(
        ...
        result = pce.analyze(problem, X, Y, order=1, fit_ratio=0.5, verbose=False)

        # For independent U(0, 1) inputs, the two linear variance terms are
        # proportional to 1^2 and 2^2, so their exact shares are 1/5 and 4/5.
        assert result.order == 1
        assert result.explained_variance is not None
        assert float(np.asarray(result.explained_variance)) > 0.99
        np.testing.assert_allclose(np.asarray(result.S1)[[0, -1]], [0.2, 0.8], atol=1e-3)
        assert float(np.max(np.asarray(result.S1)[1:-1])) < 1e-3
