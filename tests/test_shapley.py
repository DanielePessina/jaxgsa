...
def ishigami_hdmr_shapley(ishigami_data):
    X, Y = ishigami_data
    return _analyze_shapley(ishigami.PROBLEM, X, Y, backend="hdmr")
...
def ishigami_pce_shapley(ishigami_data):
    ...
    return _analyze_shapley(ishigami.PROBLEM, X, Y, backend="pce", order=9)
...
def test_linear_vs_analytical(linear_data, backend, kwargs):
    X, Y = linear_data
    result = _analyze_shapley(linear.PROBLEM, X, Y, backend=backend, **kwargs)
...
def test_pce_multi_output_matches_scalar(linear_data):
    ...
    multi = _analyze_shapley(linear.PROBLEM, X, Y2, backend="pce", order=2)
    scalar = _analyze_shapley(linear.PROBLEM, X, Y, backend="pce", order=2)
...
def test_multi_output_shapes(linear_data):
    ...
    Y2 = jnp.stack([Y, 2.0 * Y + 1.0], axis=1)
    result = _analyze_shapley(linear.PROBLEM, X, Y2, backend="hdmr")
...
def test_pce_order_reduction_surfaced():
    ...
    with pytest.warns(UserWarning, match="order reduced"):
        result = _analyze_shapley(ishigami.PROBLEM, X, Y, backend="pce", order=7)
...
def test_correlative_reduces_to_structural_under_independence():
    ...
    folded = _analyze_shapley(problem, X, Y, backend="hdmr", include_correlative=True, m=5)
    structural = _analyze_shapley(problem, X, Y, backend="hdmr", include_correlative=False, m=5)
...
def test_indices_route_constant_output_is_nan(ishigami_data):
    """``shapley.indices`` (the traceable route) masks constant slices too.

    ``indices`` shares the variance allocation with ``analyze`` but has no
    access to the result diagnostics, so the constant mask must be computed
    in the allocation itself.
    """
    X, _ = ishigami_data
    Yc = jnp.full(X.shape[0], 0.1)
    Sh, S1, ST = jaxgsa.shapley.indices(ishigami.PROBLEM, X, Yc, backend="pce", order=2)
    assert np.all(np.isnan(np.asarray(Sh)))
    assert np.all(np.isnan(np.asarray(S1)))
    assert np.all(np.isnan(np.asarray(ST)))
...
def test_multi_output_explained_variance_per_slice(ishigami_data):
    ...
    result = _analyze_shapley(ishigami.PROBLEM, X, Y2, backend="hdmr")
