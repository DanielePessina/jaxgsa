...
Both ``emulate_pce`` and ``emulate_hdmr`` accept a ``batch_size`` kwarg that
...
def test_pce_batched_matches_single_shot(problem, shape, batch_size):
    ...
    single = pce.emulate(result, X_new, batch_size=N_NEW)
    batched = pce.emulate(result, X_new, batch_size=batch_size)
...
def test_hdmr_batched_matches_single_shot(problem, shape, batch_size):
    ...
    single = hdmr.emulate(result, X_new, batch_size=N_NEW)
    batched = hdmr.emulate(result, X_new, batch_size=batch_size)
...
def test_hdmr_third_order_batched_matches_single_shot(problem):
    ...
    single = hdmr.emulate(result, X_new, batch_size=N_NEW)
    batched = hdmr.emulate(result, X_new, batch_size=64)
...
def test_auto_batch_default_matches_explicit(problem, shape):
    ...
    np.testing.assert_allclose(
        np.asarray(pce.emulate(r_pce, X_new)),
        np.asarray(pce.emulate(r_pce, X_new, batch_size=N_NEW)),
        rtol=1e-5,
        atol=1e-6,
    )
    ...
    np.testing.assert_allclose(
        np.asarray(hdmr.emulate(r_hdmr, X_new)),
        np.asarray(hdmr.emulate(r_hdmr, X_new, batch_size=N_NEW)),
        rtol=1e-5,
        atol=1e-6,
    )
...
def test_prenormalized_hdmr_batched(problem):
    ...
    single = hdmr.emulate(result, X_new, batch_size=N_NEW)
    batched = hdmr.emulate(result, X_new, batch_size=32)
...
def test_invalid_batch_size_raises(problem):
    ...
    with pytest.raises(ValueError, match="batch_size"):
        pce.emulate(result, _x_new(), batch_size=0)
...
def test_pce_batched_jit_compatible(problem):
    """emulate_pce stays jit-traceable when batching kicks in."""
    X, Y = _make_xy("scalar")
    result = pce.analyze(problem, X, Y, order=3)
    X_new = _x_new()

    fn = jax.jit(lambda x: pce.emulate(result, x, batch_size=100))
    np.testing.assert_allclose(
        np.asarray(fn(X_new)),
        np.asarray(pce.emulate(result, X_new, batch_size=N_NEW)),
        rtol=1e-6,
        atol=1e-6,
    )
