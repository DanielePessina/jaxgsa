...
class TestConstantCurve:
    """Regression: a constant curve must yield NaN, not FFT-round-off garbage.

    A non-zero constant curve leaves FFT round-off behind, so its total
    variance V is tiny but nonzero and the old ``V == 0`` guard missed it:
    S1 and ST came out as ratios of round-off residues (S1 ~ 0.013,
    ST ~ 0.722 for ``full((n,), 3.0)``). The kernel now tests
    ``max == min`` and reports NaN, as the per-curve warning promises.
    """

    def test_constant_curve_yields_nan_and_warns(self):
        sr = sample(ishigami.PROBLEM, n_per_curve=257, M=4, seed=7)
        Y = ishigami.evaluate(jnp.asarray(sr.samples))
        Y = Y.at[: sr.n_per_curve].set(3.0)
        ...
        assert np.all(np.isfinite(S1[1:]))
        assert np.all(np.isfinite(ST[1:]))

    def test_non_constant_curve_stays_finite(self):
        """A varying curve must not be over-masked by the constant check."""
        sr = sample(ishigami.PROBLEM, n_per_curve=257, M=4, seed=7)
        Y = ishigami.evaluate(jnp.asarray(sr.samples))
        result = analyze(sr, Y)
        S1 = np.asarray(result.S1)
