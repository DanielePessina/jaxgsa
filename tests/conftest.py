...
def ishigami_sobol_result():
    ...
    sr = gsax.sobol.sample(ishigami.PROBLEM, n_samples=2**14 * 8, seed=42, verbose=False)
    Y = ishigami.evaluate(jnp.asarray(sr.samples))
    return gsax.sobol.analyze(sr, Y)


@pytest.fixture(scope="session")
def linear_sobol_result():
    """Linear additive model Sobol analysis result (session-scoped)."""
    sr = gsax.sobol.sample(linear.PROBLEM, n_samples=2**13 * 5, seed=123, verbose=False)
