...
def test_2d_input_shape():
    ...
    sr = gsax.sobol.sample(p, n_samples=256, seed=42, verbose=False)
    K = 3
...
def test_single_param():
    ...
    sr = gsax.sobol.sample(p, n_samples=256, calc_second_order=False, seed=42, verbose=False)
    Y = jax.random.normal(jax.random.key(1), (sr.n_total,))
    result = gsax.sobol.analyze(sr, Y)
    assert result.S1.shape == (p.num_vars,)
    assert result.ST.shape == (p.num_vars,)
...
def _bootstrap_result(Y_shape_suffix, calc_second_order=True):
    ...
    return gsax.sobol.analyze(sr, Y, num_resamples=50, key=jax.random.key(99)), p
