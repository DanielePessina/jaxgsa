...
- **"Some of my inputs are categorical."** Declare them with `{"dist": "categorical", "probs": [...]}` — samples then carry integer level codes. Three methods handle unordered levels correctly: [Sobol'](#sobol-indices-via-saltelli-sampling) (the Saltelli column-swap scheme is distribution-agnostic), [Borgonovo delta](#borgonovo-delta-density-based-sensitivity), and [optimal transport](#optimal-transport-wasserstein-based-sensitivity) (both condition on one class per level). Every other method refuses with a `ValueError`, because its indices would depend on the arbitrary code order. See [Categorical Inputs](/examples/categorical-inputs).
...
| Categorical inputs | Supported (column swaps on level codes) | Rejected | Rejected | Rejected | Rejected | Rejected | Rejected | Rejected | Rejected | Supported (one class per level) | Supported (one class per level) |
