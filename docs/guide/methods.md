...
- **"My inputs are correlated."** Sobol', PCE, eFAST, DGSM, and Morris all assume independent inputs. Use HDMR (which separates structural from correlation-induced variance), Shapley effects with `include_correlative=True` (which folds HDMR's correlative variance into a single per-parameter score, read empirically from your samples), or HSIC / PAWN / Borgonovo delta (which make no independence assumption).
...
| Input independence | Assumed | Handled via ANCOVA decomposition | Assumed | Assumed by default; optional ANCOVA correlative mode (`include_correlative`, HDMR backend) | Assumed | Assumed | Assumed | Not assumed | Not assumed | Not assumed |
...
**Independent inputs by default**: the allocation above assumes independence, splitting each interaction's variance equally among its participants. gsax also offers an **optional** correlation-aware mode for the HDMR backend — pass `include_correlative=True` — which folds in the correlative ANCOVA variance $S_b = \mathrm{Cov}(f_j, \sum_{k \neq j} f_k)/\mathrm{Var}(Y)$, reading the correlation *empirically from the supplied $X$* rather than from a declared joint distribution. Two things change when it is on: individual indices may become **negative** and the bracketing $S_1 \leq \mathrm{Sh} \leq S_T$ need not hold (both expected under dependence, not bugs), while efficiency ($\sum_i \mathrm{Sh}_i = 1$) is preserved. Read the correlated numbers as a variance *attribution*: this is the given-data ANCOVA notion (Li et al., 2010), not the conditional-variance Shapley estimator (Song, Nelson & Staum, 2016), which requires a joint distribution and conditional sampling and remains future work — the two agree only under independence.
...
- **`backend="hdmr"`** fits the RS-HDMR B-spline surrogate and uses the structural ($S_a$) variances of its component functions as the partial variances $V_u$, truncated at `maxorder`. With `include_correlative=True` it instead uses $S_a + S_b$ per term, so the allocation credits variance shared through input correlation; `explained_variance` then reports the true emulator $R^2 = \mathrm{Var}(\hat{Y})/\mathrm{Var}(Y)$ rather than the structural sum.
