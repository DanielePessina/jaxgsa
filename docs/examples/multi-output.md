...
time_result = gsax.sobol.analyze(sampling_result, Y_time)
snapshot_result = gsax.sobol.analyze(sampling_result, Y_snapshot)
...
scalar_result = gsax.sobol.analyze(sampling_result, Y_scalar)
print(scalar_result.S1.shape)    # (D,)

# Time-series with one output
Y_one_output = Y_time[:, :, :1]  # (N, T, 1)
one_output_result = gsax.sobol.analyze(sampling_result, Y_one_output)
...
- The same shape rules apply to `gsax.hdmr.analyze()`.
