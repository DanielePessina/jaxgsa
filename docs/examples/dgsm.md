...
The full script is [`examples/dgsm_benchmark.py`](https://github.com/danielepessina/jaxgsa/blob/master/examples/dgsm_benchmark.py), run with `uv run python examples/dgsm_benchmark.py`.
...
## Validation on the benchmark functions

`examples/dgsm_benchmark.py` runs the same call against two benchmarks with
closed-form answers: Ishigami, which is strongly nonlinear with a pure
interaction, and a purely additive linear model. The point is not the API,
which the scalar example covers, but what the bracket does on each kind of
function. Both problems use uniform marginals, so the script opens with the
warning the previous section explains — the lower values are estimates, not
floors — and adds a second one: every upper bound here exceeds 1.

**Ishigami.** The unbatched helper is defined once, in the `(3,) -> ()`
shape, at the only place it is used:
...
X_ish = jaxgsa.sampling.monte_carlo(ishigami.PROBLEM, n=50_000, seed=42)
result_ish = jaxgsa.dgsm.analyze(ishigami.PROBLEM, ishigami_fn, jnp.asarray(X_ish))
```

The script prints the nu/sigma table and the bracket:

| Param | $\nu_i$ | $\sigma_i$ | Lower | $S_T$ | Upper |
| --- | ---: | ---: | ---: | ---: | ---: |
| x1 | 7.7162 | +0.0248 | 0.0001 | 0.5576 | 2.2162 |
| x2 | 24.4046 | +0.0034 | 0.0000 | 0.4424 | 7.0094 |
| x3 | 11.0375 | +0.0009 | 0.0000 | 0.2437 | 3.1702 |

Two readings. First, the bracket contains the analytical $S_T$ for all three
parameters: 0.5576, 0.4424, and 0.2437 all sit between their lower and upper
values. Second, look at x3: $\sigma_3 = 0.0009$, so the lower estimate is
0.0000, yet $S_T = 0.2437$ and the upper bound is 3.1702. x3 enters only
through the interaction term $B x_3^4 \sin(x_1)$, whose derivative in $x_3$
is odd; its mean over the symmetric domain is near zero, which is exactly
what $\sigma \approx 0$ reports. The interaction is not lost, though: it is
what keeps $\nu_3 = 11.04$ and the upper bound at 3.17.

The script also warns that every upper bound exceeds 1, the smallest being
2.22. On this benchmark the upper bound excludes nothing by itself; the
value of the run is the containment check and the $\nu$ ranking
(x2 > x3 > x1).

![DGSM lower and upper bounds bracketing the analytical total-order indices on Ishigami](./figures/dgsm_benchmark_ishigami-dgsm-bounds-vs-analytical-s-t.png)

**Linear model.** The same pattern, with coefficients $(1, 2, 3)$:
...
X_lin = jaxgsa.sampling.monte_carlo(linear.PROBLEM, n=10_000, seed=123)
result_lin = jaxgsa.dgsm.analyze(linear.PROBLEM, linear_fn, jnp.asarray(X_lin))
```

For a purely additive model the two bounds tighten onto the exact $S_T$,
which for $c = (1, 2, 3)$ is $(1, 4, 9)/14$:

| Param | $\nu_i$ | $\sigma_i$ | Lower | $S_T$ | Upper |
| --- | ---: | ---: | ---: | ---: | ---: |
| x1 | 1.0000 | +1.0000 | 0.0720 | 0.0714 | 0.0876 |
| x2 | 4.0000 | +2.0000 | 0.2882 | 0.2857 | 0.3504 |
| x3 | 9.0000 | +3.0000 | 0.6484 | 0.6429 | 0.7883 |

The lower bound lands on the exact value to within Monte Carlo noise (0.6484
vs 0.6429 for x3), because for a linear response the Kucherenko-Song
expression is exact. The upper bound keeps the uniform Poincare constant's
slack: $12/\pi^2 \approx 1.216$ times $S_T$, measured at 1.23 in this run.
Both bounds rank the inputs exactly as $S_T$ does (x3 > x2 > x1). The full
collapse of both bounds onto $S_T$ happens on Gaussian marginals, where the
Poincare constant is $\operatorname{Var}(x_i)$ rather than
$(b-a)^2/\pi^2$.

![DGSM bounds collapsing onto the exact total-order indices for the additive linear model](./figures/dgsm_benchmark_linear-model-dgsm-bounds-collapse-to-exact-s-t.png)
