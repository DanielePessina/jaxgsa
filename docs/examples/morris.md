...
run with `uv run python examples/morris_gsa.py`.
...
![Morris mu-star-sigma plane for Ishigami from 100 trajectories, marking which inputs are additive and which interact](./figures/morris_gsa_ishigami-morris-mu-sigma-plane-trajectory-r-100.png)
...
free. At `r = 100` the accounting is 400 expanded rows down to 64 unique
evaluations — `duplicates_removed=336 (84.0%)`, an 84% saving over the nominal
`r * (D + 1) = 400` runs. That saturation is specific to a coarse grid in low
dimension. At `D = 10`
...
![Morris mu-star bar chart with 95% bootstrap confidence intervals from 500 resamples](./figures/morris_gsa_mu-with-95-bootstrap-cis-500-resamples.png)

## A cheap convergence check

Sample once at the largest `r`, then confirm the ranking survives a smaller
one. The script samples at `r = 100` and downsamples to 25 trajectories:

```python
import numpy as np
import jax.numpy as jnp

sr = jaxgsa.morris.sample(PROBLEM, n_trajectories=100, seed=42, verbose=False)
Y = evaluate(jnp.asarray(sr.samples))
result = jaxgsa.morris.analyze(sr, Y, verbose=False)

sr_25, Y_25 = sr.downsample(25, np.asarray(Y))
result_25 = jaxgsa.morris.analyze(sr_25, jnp.asarray(Y_25), verbose=False)


def ranking(mu_star):
    return " > ".join(PROBLEM.names[i] for i in np.argsort(-np.asarray(mu_star)))


print(f"r=100 ranking: {ranking(result.mu_star)}   ({sr.n_runs} unique evals)")
print(f"r=25  ranking: {ranking(result_25.mu_star)}   ({sr_25.n_runs} unique evals)")
print(f"rankings stable: {ranking(result.mu_star) == ranking(result_25.mu_star)}")
```

```
r=100 ranking: x1 > x2 > x3   (64 unique evals)
r=25  ranking: x1 > x2 > x3   (53 unique evals)
rankings stable: True
```

A quarter of the trajectories still yields 53 unique evaluations, and the
ranking does not move. `rankings stable: True` is a cheap gate to put in front
of the expensive part of a screening pipeline.

![Morris mu-star at r=100 against the same design downsampled to 25 trajectories](./figures/morris_gsa_mu-stability-under-downsampling-trajectory-desig.png)
...
to connect. Radial steps can also land on the steep local slopes the grid
never visits. The derivative of `7 * sin(x2)^2` is `7 * sin(2*x2)`, with
magnitude up to 7 where the response is steepest. The fixed `delta = 2/3`
grid step always connects the same two sampled values, so it reports one
secant slope with the steep parts averaged away. The radial number is the
trustworthy one here. `num_levels` is ignored by this design, and
`scramble=True` (the default) should stay on.

![Morris mu-star for the trajectory and radial designs on the same 100-trajectory budget](./figures/morris_gsa_trajectory-vs-radial-mu-at-the-same-budget-r-100.png)
...
The two measures are two weightings of the same increments. Jansen's
total-order estimator is $E[\Delta_j^2 EE_j^2] / (2 \cdot \mathrm{Var}\,Y)$ —
with $\Delta_j$ the input step and $EE_j$ the elementary effect, the product
is just the squared output change — while Morris reports $\mu^* = E|EE_j|$.
Same increments, weighted differently: Jansen weights each by its square,
Morris by its absolute value, and that is why the ranking can shift.

Screening verdicts are design-dependent: when in doubt, keep every parameter
that either design flags as important.
...
![Sobol ST and derived Morris mu-star normalized to their own maxima, both from the same Saltelli design at zero extra model runs](./figures/morris_gsa_both-from-one-design-2048-model-runs-0-extra-256.png)
