...
The API is the same shape as the Sobol workflow: `efast.sample(...)` builds the
design, `efast.analyze(...)` consumes it. Switching methods is a one-line
change — swap the `sample` call and keep the rest.

The full script is
[`examples/efast_gsa.py`](https://github.com/danielepessina/jaxgsa/blob/master/examples/efast_gsa.py),
run with `uv run python examples/efast_gsa.py`.
...
![eFAST first-order and total-order indices on the Ishigami benchmark compared against the analytical values](./figures/efast_gsa_ishigami-efast-vs-analytical-indices.png)
...
## Higher dimensions: the Sobol G-function

The Sobol G-function is the standard high-dimensional benchmark:
`g(x) = prod_j (|4*x_j - 2| + a_j) / (1 + a_j)`, a product of independent
factors on `[0, 1]^D`. The importance vector `a` sets each input's influence
directly: `a_j = 0` is maximally influential, `a_j = 99` nearly inert. The
benchmark's default coefficients are `(0, 1, 4.5, 9, 99, 99, 99, 99)`, so the
last four inputs should come out with bars sitting near zero.

```python
import jax.numpy as jnp
from jaxgsa import efast
from jaxgsa.benchmarks.sobol_g import PROBLEM, evaluate

samples = efast.sample(PROBLEM, n_per_curve=4096, M=4, seed=123)
Y = evaluate(jnp.asarray(samples.samples))
result = efast.analyze(samples, Y)
print(result)
```
...
```
jaxgsa.efast.sample: D=8, n_curves=8, n_per_curve=4096, n_runs=32768, M=4, omega_0=511
jaxgsa.efast.analyze
  problem: D=8 (x1, x2, x3, x4, x5, x6, x7, x8)
    marginals: uniform=8
    correlation: independent
    output: N=32768 runs, T=1 x K=1 output slice
    invalid: none found in 8 search curves (policy 'raise')
  timing:
    estimator (includes compile on the first call): 0.533 s
    slice_chunk_size: auto (resolved from the memory budget)
    omega_0: 511, M: 4
  results: top 5 of 8 parameters by ST
    1. x1  ST=0.7874
    2. x2  ST=0.2424
    3. x3  ST=0.03381
    4. x4  ST=0.01047
    5. x7  ST=0.0001825
EFASTResult(S1=(8,), ST=(8,), omega_0=511, M=4)
```

Eight curves, so the bill is `8 * 4096 = 32768` runs, and the reading follows
the coefficients: `x1` dominates at `ST = 0.787`, `x2` is moderate, `x3` and
`x4` are weak, and the `a = 99` tier comes in at `ST = 0.0002` or below (the
summary shows `x7` at `0.00018`). eFAST found the "safe to fix" tier without
being told which inputs those were.

![eFAST first-order and total-order indices on the eight-input Sobol G-function against the analytical values](./figures/efast_gsa_sobol-g-function-efast-vs-analytical-indices.png)
...
A cheaper way to build a second output is to scale the first one. The script's
multi-output demo stacks the full Ishigami output with its half-amplitude copy:
...
samples = efast.sample(ishigami.PROBLEM, n_per_curve=4096, M=4, seed=7)
Y_full = ishigami.evaluate(jnp.asarray(samples.samples))
Y_multi = jnp.stack([Y_full, 0.5 * Y_full], axis=-1)  # (n_runs, 2)

result = efast.analyze(samples, Y_multi)
print(result)
```
...
jaxgsa.efast.analyze
  problem: D=3 (x1, x2, x3)
    marginals: uniform=3
    correlation: independent
    output: N=12288 runs, T=1 x K=2 output slices
    invalid: none found in 3 search curves (policy 'raise')
  timing:
    estimator (includes compile on the first call): 0.6224 s
    slice_chunk_size: auto (resolved from the memory budget)
    omega_0: 511, M: 4
  results: top 3 of 3 parameters by ST, mean over 2 output slices
    1. x1  ST=0.5507
    2. x2  ST=0.463
    3. x3  ST=0.2393
EFASTResult(S1=(2, 3), ST=(2, 3), omega_0=511, M=4)
```

A constant scale cancels in every variance ratio, so both slices return the
same indices — the top-k summary is unchanged from the scalar case. That makes
the scaled copy a way to check the multi-output path, not a way to learn
something new about the model.

![Per-output eFAST bar charts for the full Ishigami output and its half-amplitude copy](./figures/efast_gsa_ishigami-full.png)
...
## Time series on a damped oscillator

The script's time-series demo is a single-output damped oscillator,
`y(t) = A * exp(-gamma * t) * sin(omega * t)`, with amplitude on `(0.5, 2.0)`,
damping on `(0.1, 0.5)` and frequency on `(1.0, 5.0)`, sampled at 50 steps of
`t` from 0.1 to 8.0.
...
problem = Problem.from_dict(
    {"amplitude": (0.5, 2.0), "damping": (0.1, 0.5), "frequency": (1.0, 5.0)}
)

times = jnp.linspace(0.1, 8.0, 50)
...
samples = efast.sample(problem, n_per_curve=4096, M=4, seed=99)
Y = damped_oscillator(jnp.asarray(samples.samples), times)
Y = Y[..., None]  # (n_runs, T, 1)
result = efast.analyze(samples, Y)
print(result)
```
...
jaxgsa.efast.sample: D=3, n_curves=3, n_per_curve=4096, n_runs=12288, M=4, omega_0=511
jaxgsa.efast.analyze
  problem: D=3 (amplitude, damping, frequency)
    marginals: uniform=3
    correlation: independent
    output: N=12288 runs, T=50 x K=1 output slices
    invalid: none found in 3 search curves (policy 'raise')
  timing:
    estimator (includes compile on the first call): 0.2865 s
    slice_chunk_size: auto (resolved from the memory budget)
    omega_0: 511, M: 4
  results: top 3 of 3 parameters by ST, mean over 50 output slices
    1. frequency  ST=0.8588
    2. amplitude  ST=0.5996
    3. damping    ST=0.4983
EFASTResult(S1=(50, 1, 3), ST=(50, 1, 3), omega_0=511, M=4)
```

The `Y[..., None]` reshape adds the trailing `K = 1` axis, so the analyzer sees
`(n_runs, T, 1)` and returns indices of shape `(T, K, D) = (50, 1, 3)` — the
same convention the shape rules describe for a single named output. The
physics shows up in the time course: amplitude controls the early envelope. At
`t = 0.1` the exponential has barely moved, and the dataset print shows
amplitude's `S1` opening at 0.432 while damping's opens at 0.0005. Damping
decides the decay later, as `exp(-gamma * t)` collapses; frequency carries the
oscillation itself, which is why it holds the largest mean `ST` over the whole
window (0.859).

![First-order and total-order eFAST indices over time for the damped oscillator](./figures/efast_gsa_first-order-s1-t.png)
