...
Run it with `uv run python examples/batch_reactor_gsa.py`; it prints the
transcripts below and draws the three plots discussed at the end.
...
time-resolved plot in the script is for.

## The numbers behind the plots

Here is the script's analysis as a self-contained block, with the two time
...
![Batch-reactor outlet concentration trajectories from 24 Saltelli samples, filling toward steady state](./figures/batch_reactor_gsa_batch-reactor-concentration-trajectories-24-samp.png)
...
Running the script produces three figures. The steady-state bar chart shows
$S_1$ and $S_T$ at the final time step with the bootstrap 95 % confidence
intervals as error bars.

![Steady-state bar chart of first- and total-order Sobol indices for Ca0, temperature and pH at the final time step, with bootstrap 95% confidence error bars](./figures/batch_reactor_gsa_steady-state-sensitivity-ca-at-the-final-time-st.png)

The time-resolved plot traces $S_1(t)$ and $S_T(t)$
for each input with the bootstrap envelopes shaded — it is where the ranking
change described above becomes visible as two curves crossing.

![Time-resolved first-order S1(t) and total-order ST(t) Sobol indices for Ca0, temperature and pH, with shaded bootstrap envelopes](./figures/batch_reactor_gsa_first-order-s1-t.png)

The pairwise
$S_{ij}$ heatmap at the final time step rounds it out.

![Pairwise second-order Sobol index S2 heatmap at steady state between Ca0, temperature and pH](./figures/batch_reactor_gsa_steady-state-second-order-indices-s2-i-j.png)
