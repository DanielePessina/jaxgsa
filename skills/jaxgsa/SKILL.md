...
- `explained_variance` should sit near 1, and `loo_rmse` should be small next to
  ...
  it climbs as you add terms and cannot detect an overfit on its own, which is
  why both numbers are checked. `analyze` warns when `explained_variance` falls
  below 0.5 on any output slice, and separately when `loo_rmse` passes 0.71
  times `std(Y)`, the out-of-sample form of the same line. A silent run means
  both diagnostics passed. `indices()` warns about neither, because it traces.
...
- **`result.S.sum()` must be near 1 before you rank anything.** The shortfall is
  variance the decomposition did not capture, and `Sa`, `Sb`, `S`, and `ST` all
  inherit it. `analyze` warns when the per-slice sum falls below 0.5 or rises
  above 1.3, which is the Li et al. (2010, Eq. 24) precondition made loud.
  Raise `maxorder` or `m`, or accept that this model does not decompose into
  low-order terms.
...
- **Check `explained_variance` first, every time.** The effects are exact within
  ...
  at `order=10`, which matches the analytical values. The backend's own
  `analyze` warns when the fit is too poor to trust, so the warning arrives
  under `jaxgsa.pce` or `jaxgsa.hdmr` rather than `jaxgsa.shapley`. For `backend="pce"` the number is a coefficient of
...
- **The bounds can be far too loose to rank with.** On Ishigami at N=1024 the
  ...
  More samples do not fix it, because the Poincare constant sets the slack, not
  the sample size. `analyze` warns when every bound on a slice exceeds 1, which
  is the state where the bound excludes nothing. DGSM is a fast way to find
  parameters that do nothing at all. It is not a reliable ranking of the ones
  that do.
...
result.bandwidth, result.n_perms      # what the run used, also in to_dataset attrs
```
...
- **`bandwidth` is a real choice, not a tuning detail.** It multiplies the
  ...
  near-constant. Sweep it, and report the value you used. The result carries
  `bandwidth` so a stored index says what produced it, and `to_dataset` writes
  it into the dataset attributes.
...
- **In a point-cloud mode, pass `dummy=True`.** `"multivariate"` and
  ...
  irrelevant parameters visibly above zero however large `N` is. Reading those
  indices without the dummy floor will make you believe in parameters that do
  nothing, so `analyze` warns when either mode runs with `dummy=False`. Read
  `above_dummy`, which is the index with the floor subtracted.
